...
def upgrade() -> None:
    ...
    sa.CheckConstraint('(terms_agreed_at IS NULL AND terms_version IS NULL) OR (terms_agreed_at IS NOT NULL AND terms_version IS NOT NULL)', name='users_terms_both_or_none'),
    ...
    sa.CheckConstraint('(unsealed_at IS NULL AND unseal_trigger IS NULL) OR (unsealed_at IS NOT NULL AND unseal_trigger IS NOT NULL)', name='capsules_unseal_consistency'),
    ...
    op.execute("""
        CREATE TABLE storage_purchases (
            ...
            CONSTRAINT storage_purchases_stripe_payment_intent_id_unique
    ...
    sa.Column('created_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('NOW()'), nullable=False),
    sa.Column('updated_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('NOW()'), nullable=False),
    sa.CheckConstraint("plan_type IN ('free', 'paid_monthly', 'paid_yearly')", name='subscriptions_plan_type_check'),
    sa.CheckConstraint("status IN ('trialing', 'active', 'past_due', 'canceled', 'unpaid')", name='subscriptions_status_check'),
    ...
    op.create_table('capsule_members',
    sa.Column('id', sa.UUID(), server_default=sa.text('gen_random_uuid()'), nullable=False),
    ...
    sa.Column('created_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('NOW()'), nullable=False),
    sa.CheckConstraint("(status = 'active' AND joined_at IS NOT NULL) OR (status != 'active')", name='capsule_members_active_consistency'),
    sa.CheckConstraint("(status = 'deleted' AND deleted_at IS NOT NULL AND deletion_reason IS NOT NULL) OR (status != 'deleted' AND deleted_at IS NULL AND deletion_reason IS NULL)", name='capsule_members_deleted_consistency'),
    sa.CheckConstraint("deletion_reason IN ('self_exit', 'removed_by_member')", name='capsule_members_deletion_reason_check'),
    sa.CheckConstraint("status IN ('invited', 'active', 'deleted')", name='capsule_members_status_check'),
    sa.ForeignKeyConstraint(['capsule_id'], ['capsules.id'], name='capsule_members_capsule_id_fkey', ondelete='RESTRICT'),
    ...
    sa.ForeignKeyConstraint(['capsule_id'], ['capsules.id'], name='fk_delivery_addresses_capsule', ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id')
    )
    op.execute("""
    ...
    sa.Column('last_calculated_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('NOW()'), nullable=False),
    sa.Column('updated_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('NOW()'), nullable=False),
    ...
    sa.ForeignKeyConstraint(['capsule_id'], ['capsules.id'], name='storage_usage_capsule_id_fkey', ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('capsule_id', name='storage_usage_capsule_id_unique')
    )
    op.create_table('unseal_events',
    sa.Column('id', sa.UUID(), server_default=sa.text('gen_random_uuid()'), nullable=False),
    sa.Column('capsule_id', sa.UUID(), nullable=False),
    ...
    sa.ForeignKeyConstraint(['executed_by'], ['users.id'], name='fk_unseal_events_executed_by', ondelete='SET NULL'),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    sa.Column('created_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('NOW()'), nullable=False),
    sa.Column('updated_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('NOW()'), nullable=False),
    sa.CheckConstraint("(email_type IN ('unseal_notice', 'record_delivery') AND unseal_event_id IS NOT NULL) OR (email_type IN ('invite', 'bounce_warning', 'open_at_change') AND unseal_event_id IS NULL)", name='chk_email_deliveries_unseal_event_consistency'),
    sa.CheckConstraint("email_type IN ('unseal_notice', 'record_delivery', 'invite', 'bounce_warning', 'open_at_change')", name='chk_email_deliveries_email_type'),
    sa.CheckConstraint("status IN ('pending', 'sent', 'delivered', 'bounced', 'failed', 'skipped')", name='chk_email_deliveries_status'),
    sa.CheckConstraint('attempt_count BETWEEN 1 AND 3', name='chk_email_deliveries_attempt_count'),
    sa.ForeignKeyConstraint(['capsule_id'], ['capsules.id'], name='fk_email_deliveries_capsule', ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['delivery_address_id'], ['delivery_addresses.id'], name='fk_email_deliveries_delivery_address', ondelete='RESTRICT'),
    sa.ForeignKeyConstraint(['record_id'], ['records.id'], name='fk_email_deliveries_record', ondelete='RESTRICT'),
    sa.ForeignKeyConstraint(['unseal_event_id'], ['unseal_events.id'], name='fk_email_deliveries_unseal_event', ondelete='RESTRICT'),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    sa.Column('created_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('NOW()'), nullable=False),
    sa.CheckConstraint("(attachment_type = 'audio' AND duration_seconds IS NOT NULL) OR (attachment_type = 'image' AND duration_seconds IS NULL)", name='chk_record_attachments_duration'),
    sa.CheckConstraint("(attachment_type = 'image' AND width IS NOT NULL AND height IS NOT NULL) OR (attachment_type = 'audio' AND width IS NULL AND height IS NULL)", name='chk_record_attachments_dimensions'),
    ...
    sa.ForeignKeyConstraint(['record_id'], ['records.id'], name='fk_record_attachments_record', ondelete='RESTRICT'),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    op.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_users_stripe_customer_id ON users (stripe_customer_id) WHERE stripe_customer_id IS NOT NULL")
    op.execute("CREATE INDEX IF NOT EXISTS idx_users_is_deleted ON users (is_deleted) WHERE is_deleted = FALSE")
    op.execute("CREATE INDEX IF NOT EXISTS idx_users_trial_ends_at ON users (trial_ends_at) WHERE trial_ends_at IS NOT NULL AND is_deleted = FALSE")
    op.execute("CREATE INDEX IF NOT EXISTS idx_users_anonymize_scheduled_at ON users (anonymize_scheduled_at) WHERE anonymize_scheduled_at IS NOT NULL")

    # --- capsules ---
    op.execute("CREATE INDEX IF NOT EXISTS idx_capsules_open_at_unsealed ON capsules (open_at) WHERE unsealed_at IS NULL")

    # --- capsule_members ---
    op.execute("CREATE INDEX IF NOT EXISTS idx_capsule_members_capsule_id ON capsule_members (capsule_id)")
    op.execute("CREATE INDEX IF NOT EXISTS idx_capsule_members_user_id ON capsule_members (user_id) WHERE user_id IS NOT NULL")
    op.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_capsule_members_capsule_email ON capsule_members (capsule_id, LOWER(invited_email))")
    op.execute("CREATE INDEX IF NOT EXISTS idx_capsule_members_active ON capsule_members (capsule_id, status) WHERE status = 'active'")

    # --- records ---
    op.execute("CREATE INDEX IF NOT EXISTS idx_records_capsule_window ON records (capsule_id, window_ends_at) WHERE window_ends_at IS NOT NULL")
    ...
    op.execute("CREATE INDEX IF NOT EXISTS idx_delivery_addresses_is_active ON delivery_addresses (capsule_id, is_active) WHERE is_active = TRUE")
    op.execute("CREATE INDEX IF NOT EXISTS idx_delivery_addresses_tag ON delivery_addresses (capsule_id, tag)")
    op.execute("CREATE INDEX IF NOT EXISTS idx_delivery_addresses_added_by ON delivery_addresses (added_by) WHERE added_by IS NOT NULL")
    op.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_delivery_addresses_email_capsule ON delivery_addresses (capsule_id, LOWER(email))")

    # --- subscriptions ---
    op.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_subscriptions_user_id ON subscriptions (user_id)")
    op.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_subscriptions_stripe_subscription_id ON subscriptions (stripe_subscription_id) WHERE stripe_subscription_id IS NOT NULL")
    op.execute("CREATE INDEX IF NOT EXISTS idx_subscriptions_status_active ON subscriptions (status) WHERE status IN ('trialing', 'active', 'past_due')")
    op.execute("CREATE INDEX IF NOT EXISTS idx_subscriptions_trial_end ON subscriptions (trial_end) WHERE trial_end IS NOT NULL")

    # --- storage_purchases ---
    op.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_storage_purchases_stripe_payment_intent_id ON storage_purchases (stripe_payment_intent_id)")
    ...
    op.execute("CREATE INDEX IF NOT EXISTS idx_unseal_events_notification_pending ON unseal_events (capsule_id, notification_sent_at) WHERE notification_sent_at IS NULL")

    # --- email_deliveries ---
    op.execute("CREATE INDEX IF NOT EXISTS idx_email_deliveries_capsule_id ON email_deliveries (capsule_id)")
    op.execute("CREATE INDEX IF NOT EXISTS idx_email_deliveries_unseal_event_delivery ON email_deliveries (unseal_event_id, delivery_address_id) WHERE unseal_event_id IS NOT NULL")
    op.execute("CREATE INDEX IF NOT EXISTS idx_email_deliveries_retry_candidates ON email_deliveries (status, attempt_count, last_attempted_at) WHERE status = 'failed' AND attempt_count < 3")
    op.execute("CREATE INDEX IF NOT EXISTS idx_email_deliveries_delivery_address ON email_deliveries (delivery_address_id, status)")
    op.execute("CREATE INDEX IF NOT EXISTS idx_email_deliveries_record_id ON email_deliveries (record_id) WHERE record_id IS NOT NULL")
    ...
    op.execute("""COMMENT ON TABLE users IS 'Firebase Auth 認証済みユーザーのプロファイル・課金・状態管理。物理削除禁止（論理削除のみ）。退会後5年で PII 匿名化（A案採用）。'""")
    op.execute("""COMMENT ON COLUMN users.anonymize_scheduled_at IS '退会5年後の PII 匿名化予約日時。匿名化対象: display_name / email / firebase_uid / stripe_customer_id 等。Vercel Cron バッチで実施。records は永続保持する（B-5 A案）。'""")
    op.execute("""COMMENT ON TABLE capsules IS 'タイムカプセル（グループ）本体。開封日・封印状態・創設者を管理する。'""")
    op.execute("""COMMENT ON TABLE capsule_members IS 'タイムカプセルへの参加関係。invited / active / deleted の3状態で管理する。'""")
    op.execute("""COMMENT ON TABLE records IS 'タイムカプセルへの投稿本文。INSERT only（変更・削除禁止）。window_ends_at は GENERATED ALWAYS AS で自動計算（トリガー不要）。'""")
    op.execute("""COMMENT ON TABLE record_attachments IS '記録に紐付く画像・音声の S3 参照メタデータ。byte_size の合計がストレージ消費量の基準。INSERT only（変更・削除禁止）。'""")
    op.execute("""COMMENT ON TABLE delivery_addresses IS '開封時の配信先メールアドレスリスト。バウンス状態・タグフィルタ・追加者追跡を管理。email は LOWER() で正規化済み。'""")
    op.execute("""COMMENT ON TABLE subscriptions IS 'ユーザーのプラン契約状態の真実の源（source of truth）。Stripe サブスクリプションと 1:1 対応。'""")
    op.execute("""COMMENT ON TABLE storage_purchases IS '追加ストレージ買い切り購入履歴（10GB / 1,500 円）。取り消し不可・不変テーブル。expires_at は GENERATED ALWAYS AS で自動計算。'""")
    op.execute("""COMMENT ON TABLE storage_usage IS 'カプセル単位のストレージ使用量スナップショット。record_attachments INSERT トリガーにより即時更新。'""")
    op.execute("""COMMENT ON TABLE unseal_events IS 'タイムカプセル開封イベントの記録（1 開封 = 1 レコード）。INSERT only。監査ログとして機能。executed_by は users(id) 参照（B-1 対応）。'""")
    op.execute("""COMMENT ON COLUMN unseal_events.executed_by IS '手動開封時の実行者（users.id）。自動開封時は NULL。users は物理削除禁止のため20年後も追跡可能。'""")
    op.execute("""COMMENT ON TABLE email_deliveries IS 'メール配信ログ（開封通知・記録配信・招待・バウンス警告・開封日変更）。リトライ管理（最大 3 回・24 時間以内）。'""")
...
def downgrade() -> None:
    ...
    op.execute("DROP INDEX IF EXISTS idx_storage_purchases_stripe_payment_intent_id")
    ...
    op.execute("DROP INDEX IF EXISTS idx_subscriptions_stripe_subscription_id")
